...
Data file:
- [californiatod.csv](lab/californiatod.csv)
- [R markdown](lab/lab01.Rmd)
...
Install the R Software Go to http://cran.rstudio.com/ and click on Download R for Windows (or OSX), click on install R for the first time and download R. When the file is done downloading, double click the “R-3.2.4-win.exe” to install the file and select Run, and continue with the installation wizard.
...
Downlad the [californiatod.csv](lab/californiatod.csv) file and save in your personal folder or C: drive. Open up RStudio, click Tools –> Import Dataset –> From Local File and navigate to your folder and click the csv. Make sure the Heading option is marked as “Yes” and press Ok.

```{r}
#install.packages(readr)
library(readr)
catod <- read_csv("californiatod.csv")
```
...
library(scales)
p2 <- ggplot(catod, aes(x=transit, y = houseval))
...
table(catod$region, catod$railtype)

catod %>% group_by(region, railtype) %>%
  summarise(avg_transit = mean(transit))
```

The dplyr package is designed to perform table operations on dataframes for data manipulation and some summarization. The preceding syntax told R to take our catod dataframe, group by the variables for region and railtype, and then calculate the mean transit usage from there. This is similar to the kind of pivot table operations one can do in excel.
