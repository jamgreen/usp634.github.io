...
Load the eb_pums dataset using the **load()** function.

```{r, warning=FALSE, message=FALSE}

setwd("C:/Users/elmue/Desktop/Data1_labs_homeworks/Labs")

load("eb_pums_2000.RData")
...
names(pums)[6] <- "deailed.meta.area"
```
...
xtabs.TR <- xtabs(~ mode + racehisp, data = pums, 
...
prop.table(xtabs(~ mode + racehisp, data = pums, subset = !is.na(mode), drop.unused.levels = TRUE), 2)
...
chisq.test(xtabs(~mode + racehisp, data = pums, subset = !is.na(mode), drop.unused.levels = TRUE))
```
